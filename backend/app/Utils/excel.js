...
const {downloadDir, downloadHost, excelFolder} = require('../../config');
...
module.exports = {
  ...
  exportExcel(fileName, data, options) {
    ...
    let relativePath = path.join(excelFolder, fileName);
    let fullPath = path.join(downloadDir, relativePath);
    relativePath = downloadHost + path.join('/', relativePath);
