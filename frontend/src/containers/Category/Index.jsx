...
import View from 'views/Category/Index'
...
class Index extends BaseContainer {
  ...
  onFetchData(state) {
    this.props.dispatch(CategoryAction.fetchAll(state));
  }
  ...
  onDeleteData(selectedIds){
    this.props.dispatch(CategoryAction.delete({
      ids: selectedIds
    }))
    .then(result => {
      if(!result.error){
...
const mapStateToProps = state => {
  ...
  return {
    data: selector(state, "category.list", {}),
  }
}
