...
class Index extends BaseView {
      ...
      columns: [
        {
          ...
          formaterComponent: (data) =>{
            return this.textLink(data.value, `/staffs/${data.row._id}`)
          }
        },
        {
          name: 'name',
  ...
  renderActionsColumn(data){
    let _id = data.value || ''
    return <IconButton onClick={() => this.goto(`/staffs/${_id}`)}>
      <Icon>edit</Icon>
    </IconButton>
  }
  ...
  renderToolbarActions() {
    return [
      <Tooltip title={I18n.t("Tooltip.create")} key="create">
        <IconButton onClick={() => this.goto("/staffs/create")}>
  ...
  renderSelectedActions(selectedIds){
    return <React.Fragment>
      ...
      <Tooltip title={I18n.t("Tooltip.delete")} key="salary">
      <IconButton key="delete" onClick={() => this.goto(`/staffs/salary/${selectedIds.join(",")}`)}>
