...
class Index extends BaseView {
      ...
      columns: [
        {
          ...
          formatterComponent: (data) =>{
            return this.textLink(data.value, `/products/${data.row._id}`)
          }
        },
        {
          name: 'name',
        ...
        {
          name: 'category.name',
          title: I18n.t('Table.header.category.name'),
          formatterComponent: data => {
            return this.getData(data, "row.category.name", "no name")
          }
        },
        {
          name: 'price',
          title: I18n.t('Table.header.price'),
        },
        {
          name: 'insert.when',
  ...
  renderActionsColumn(data){
    let _id = data.value || ''
    return <IconButton onClick={() => this.goto(`/products/${_id}`)}>
      <Icon>edit</Icon>
    </IconButton>
  }
  ...
  renderToolbarActions() {
    return [
      <Tooltip title={I18n.t("Tooltip.create")} key="create">
        <IconButton onClick={() => this.goto("/products/create")}>
